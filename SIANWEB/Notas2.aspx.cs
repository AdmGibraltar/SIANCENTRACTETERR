    ...
    public partial class Notas2 : System.Web.UI.Page
    {
        ...
        protected void cmbProductoDet_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    new CN_CatProducto().ConsultaProducto(ref prd, session.Emp_Cnx, session.Id_Emp, session.Id_Cd_Ver, session.Id_Cd_Ver, Convert.ToInt32(txtProducto.Value.HasValue ? (sender as RadNumericTextBox).Value : -1));
                }
                catch (Exception ex)
                {
        ...
        private void LLenarFormNotaCargo(int Id_Emp, int Id_Cd, int Id_Nca)
        {
            ...
            //new CN_CapNotaCargo().ConsultarAdenda(sesion.Id_Emp, sesion.Id_Cd_Ver, Id_Nca, "3", "4", ref listCabT, ref listDetT, sesion.Emp_Cnx);
