...
namespace SIANWEB
{
    public partial class Rep_InvAnalisisRemisiones : System.Web.UI.Page
    {
        #region Variables
        ...
        private void Abrir_Reporte(bool a_pantalla)
        {
            try
            {
                ...
                CN__Comun cn_comun = new CN__Comun();
                if (rblNivel.SelectedValue == "P")  // si es reporte de detalle debe elegir un CDI o CDC
                {
                    if (cmbCDI.SelectedValue == "-1")
                    {
                        Alerta("Para este tipo de reporte se debe seleccionar una sucursal en el filtro CD.");
                        return;
                    }
                }
                else
                {        // si es reporte general no  debe elegir ningún  CDI o CDC
                    if (cmbCDI.SelectedValue != "-1")
                    {
                        Alerta("Para el reporte General no debe elegir ningúna sucursal en el  filtro CD.");
                        return;
                    }
                }
                ...
                ALValorParametrosConsignacion.Add(this.cmbCDI.SelectedValue == "-1" ? "100" : this.cmbCDI.SelectedValue );//nombre sucursal
                ...
                ALValorParametrosInternos.Add(this.cmbCDI.SelectedValue == "-1" ?  Cd_Nombre :this.cmbCDI.SelectedValue + " - " + this.cmbCDI.Text );//nombre sucursal
               ...
               // ALValorParametrosInternos.Add(dpFechafin.SelectedDate);//fecha final
                ...
                if (a_pantalla)
                {
                    if (rblNivel.SelectedValue == "P") //detalle
                    {
                ...
                if (rblNivel.SelectedValue == "D")  // si es reporte de detalle debe elegir un CDI o CDC
                {
                    
                    //Ejecutar el reporte Gral2  primero y luego el Gral

                    //try
                    //{

                    //    CN_CapRemision clsCatCompensacion = new CN_CapRemision();

                    //    int verificador;
                    //    verificador = 0;
                    //    sesion.Emp_Cnx = sesion.Emp_Cnx;
                    //    clsCatCompensacion.ReporteAnalisisdeRemisiones(sesion.Id_Emp, fechainicial, fechafinal, sesion.Emp_Cnx, ref verificador);
                    //    if (verificador == 0)
                    //    {
                    ...
                    //poner un wait si es necesario 


                    instance = typeof(LibreriaReportes.Rep_ControlRemisionesGral);
                    ImprimirXLSGral(ALValorParametrosGeneral, instance );
