...
namespace SIANWEB 
{
    public partial class CapFacturaAlmacenCobro_Lista : System.Web.UI.Page
    {
        #region Variables
        ...
        public List<FacturaAlmacenCobro> listFacturaAlmacenCobro;
        ...
        protected void rgFacturaAlmacenCobro_NeedDataSource(object source, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
        {
            try
            {
                ...
                {
                    //Llenar Grid
                    rgFacturaAlmacenCobro.DataSource = this.GetList();
                }
            }
            catch (Exception ex)
            {
                DisplayMensajeAlerta(string.Concat(ex.Message, "rgFacturaAlmacenCobro_NeedDataSource"));
            }
        }

        protected void rgFacturaAlmacenCobro_PageIndexChanged(object source, GridPageChangedEventArgs e)
        {
            try
            {
                ErrorManager();
        ...
        protected void rgFacturaAlmacenCobro_ItemCommand(object source, GridCommandEventArgs e)
        {
            ...
            try
            {
                ...
                if (item >= 0)
                {
                    int Id_Emp = Convert.ToInt32(rgFacturaAlmacenCobro.Items[item]["Id_Emp"].Text);
                    int Id_Cd = Convert.ToInt32(rgFacturaAlmacenCobro.Items[item]["Id_Cd"].Text);
                    int Id_Fac= Convert.ToInt32(rgFacturaAlmacenCobro.Items[item]["Id_Fac"].Text);
                    string estatus = rgFacturaAlmacenCobro.Items[item]["Fac_Estatus"].Text;
                    string[] datePart = rgFacturaAlmacenCobro.Items[item]["Fac_Fecha"].Text.Split(new char[] { '/' });
                    DateTime fechaFacturaAlmacenCobro = new DateTime(Convert.ToInt32(datePart[2]), Convert.ToInt32(datePart[1]), Convert.ToInt32(datePart[0]));

                    switch (e.CommandName.ToString())
                    {
                        case "Eliminar":
                            mensajeError = "CapFacturaAlmacenCobro_delete_error";
                            if (estatus.ToUpper() == "B")
                            {
                                this.DisplayMensajeAlerta("CapFacturaAlmacenCobro_EsBaja");
                            }
                            else
                            {
                                if (_PermisoEliminar)
                                {
                                    this.CancelarFacturaAlmacenCobro(Id_Emp, Id_Cd, Id_Fac);
                                    this.DisplayMensajeAlerta("CapFacturaAlmacenCobro_delete_ok");
                                }
                                else
                                {
                        ...
                        case "Modificar":
                            if (estatus.ToUpper() != "B")
                            {
                                ...
                                {
                                    RAM1.ResponseScripts.Add(string.Concat(@"AbrirVentana_FacturaAlmacenCobro_Edicion('", Id_Emp, "','", Id_Cd, "','", Id_Fac, "','" + estatus.ToUpper() + "')"));
                                }
                                else
                                {
                            ...
                            {
                                this.DisplayMensajeAlerta("CapFacturaAlmacenCobro_Modificar_Denegado");
                            }
                            break;
                        case "Imprimir":
                            mensajeError = "CapFacturaAlmacenCobro_print_error";
                            if (estatus.ToUpper() == "I" || estatus.ToUpper() == "C")
                            {
                                if (_PermisoImprimir)
                                {
                                    this.ImpresionFacturaAlmacenCobro(Id_Emp, Id_Cd, Id_Fac);
                                }
                                else
                                {
                            ...
                            {
                                this.DisplayMensajeAlerta("CapFacturaAlmacenCobro_EstatusIncorrecto");
                            }
                            break;
                    }
                }
                ...
                {
                    rgFacturaAlmacenCobro.MasterTableView.Columns.FindByUniqueName("Editar").Visible = false;
                    rgFacturaAlmacenCobro.MasterTableView.Columns.FindByUniqueName("Eliminar").Visible = false;
                    rgFacturaAlmacenCobro.MasterTableView.Columns.FindByUniqueName("Imprimir").Visible = false;

                    if (e.CommandName.ToString().ToUpper().Contains("PDF"))
                    {
                        rgFacturaAlmacenCobro.MasterTableView.Columns.FindByUniqueName("Cte_NomComercial").HeaderStyle.Width = Unit.Pixel(200);
                        rgFacturaAlmacenCobro.MasterTableView.Columns.FindByUniqueName("Ncr_Fecha").HeaderStyle.Width = Unit.Pixel(70);
                    }
                }
        ...
        protected void rgFacturaAlmacenCobro_ItemDataBound(object sender, GridItemEventArgs e)
        {
            try
            {
                //GridItem cmdItem = rgFacturaAlmacenCobro.MasterTableView.GetItems(GridItemType.CommandItem)[0];
        ...
        private void DisplayMensajeAlerta(string mensaje)
        {
            ...
            if (mensaje.Contains("CapFacturaAlmacenCobro_MOdificar_Movimiento5"))
                Alerta("Movimiento generado en forma automática. Imposible modificar este documento");
            else
            if (mensaje.Contains("rgFacturaAlmacenCobro_NeedDataSource"))
            ...
            if (mensaje.Contains("CapFacturaAlmacenCobro_Modificar_Denegado"))
                Alerta("Imposible modificar el documento");
            else
            if (mensaje.Contains("CapFacturaAlmacenCobro_delete_ok"))
                Alerta("El registro de factura a revisión o cobro se ha dado de baja (estatus \"B\") correctamente");
            else
            if (mensaje.Contains("CapFacturaAlmacenCobro_EstatusIncorrecto"))
                Alerta("No se puede realizar la operación. El estatus es incorrecto");
            else
            if (mensaje.Contains("CapFacturaAlmacenCobro_EsBaja"))
                Alerta("El registro de factura a revisión o cobro ya está dado de baja");
            else
            if (mensaje.Contains("CapFacturaAlmacenCobro_delete_error"))
