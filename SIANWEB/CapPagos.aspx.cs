    ...
    public partial class CapPagos : System.Web.UI.Page
    {
        ...
        protected void RgDet_ItemCreated(object sender, GridItemEventArgs e)
        {
            try
            {
                ...
                {
                    GridEditableItem item = e.Item as GridEditableItem;
                    if (item.FindControl("rgSerie") != null)
                        (item.FindControl("rgSerie") as RadTextBox).Focus();
                }
            }
            catch (Exception ex)
            {
        ...
        protected void rgTxtCliente_TextChanged(object source, EventArgs e)
        {
            try
            {
                ...
                if (dr_ar.Length == 0)
                {
                    LimpiarDetalle(rgTerr, rdFecha, rgTerr, rgCteNombre, rgEstatus, rgtxtImporte, rgPagado);
                    if (Movimiento == 1)
                    {
                        Factura ficha = new Factura();
                        ficha.Id_Emp = sesion.Id_Emp;
        ...
        private static void LimpiarDetalle(RadNumericTextBox rgTerr, RadDatePicker rdFecha, RadNumericTextBox rgCte, RadTextBox rgCteNombre, RadTextBox rgEstatus, RadNumericTextBox rgImporte, RadNumericTextBox rgPagado)
        {
