...
namespace SIANWEB
{
    public partial class CapRemisiones_Lista : System.Web.UI.Page
    {
        ...
        private void baja(ref GridCommandEventArgs e)
        {
            ...
            Remision remision = new Remision();
            remision.Id_Emp = int.Parse(rgPedido.MasterTableView.Items[e.Item.ItemIndex]["Id_Emp"].Text);
            remision.Id_Cd = int.Parse(rgPedido.MasterTableView.Items[e.Item.ItemIndex]["Id_Cd"].Text);
            remision.Id_Rem = int.Parse(rgPedido.MasterTableView.Items[e.Item.ItemIndex]["Id_Rem"].Text);
            remision.Rem_Fecha = fecha;
            remision.Rem_Estatus = "B";
            remision.Id_Tm = int.Parse(rgPedido.MasterTableView.Items[e.Item.ItemIndex]["Id_Tm"].Text);
            remision.Id_Cte = Convert.ToInt32(rgPedido.MasterTableView.Items[e.Item.ItemIndex]["Id_Cte"].Text);
            if (int.Parse(rgPedido.MasterTableView.Items[e.Item.ItemIndex]["Id_Ped"].Text) > 0)
            {
                remision.Id_Ped = int.Parse(rgPedido.MasterTableView.Items[e.Item.ItemIndex]["Id_Ped"].Text);
            }
            else
            {
                remision.Id_Ped = -1;
