...
namespace SIANWEB
{
    public partial class Notas : System.Web.UI.Page
    {
                    ...
                    switch (e.CommandName.ToString())
                    {
                        case "Eliminar":
                            ...
                            else
                            {
                                if ((estatus.Contains("Impreso") || estatus.Contains("Capturado")) && (fechaNotaCargo >= sesion.CalendarioIni && fechaNotaCargo <= sesion.CalendarioFin))
                                {
                                    if (_PermisoEliminar)
                                    {
                                        this.CancelarNotaCargo(Id_Emp, Id_Cd, Id_Nca);
                                        this.rgNotaCargo.Rebind();
                        ...
                        case "Modificar":
                            ...
                            if (_PermisoModificar)
                                RAM1.ResponseScripts.Add(string.Concat(@"AbrirVentana_NotaCargo_Edicion('", Id_Emp, "','", Id_Cd, "','", Id_Nca, "','", notaModificable, "')"));
                            else
                        ...
                        case "Imprimir":
                            ...
                            {
                                if (_PermisoImprimir)
                                    this.ImprimirNotaCargo(Id_Emp, Id_Cd, Id_Nca);
                                else
        ...
        private void CancelarNotaCargo(int Id_Emp, int Id_Cd, int Id_Nca)
        {
            Sesion sesion = (Sesion)Session["Sesion" + Session.SessionID];
            NotaCargo notaCargo = new NotaCargo();
            int verificador = 0;
        ...
        private void ImprimirNotaCargo(int Id_Emp, int Id_Cd, int Id_Nca)
        {
            try
            {
                Sesion sesion = (Sesion)Session["Sesion" + Session.SessionID];
                ...
                //new CN_CapNotaCargo().ConsultarAdenda(Id_Emp, Id_Cd, Id_Nca, "3", "4", ref listCabT, ref listDetT, sesion.Emp_Cnx);
                ...
                //if (notaCargo.Nca_Sello != string.Empty)
                //{ //Abre el XML y carga el PDF de la factura
                //    object resultado = null;
                //    cn_notaCargo.ConsultarNotaCargoSAT(ref notaCargo, sesion.Emp_Cnx, ref resultado);
                ...
                //        // ------------------------------------------------------------------------------------------------
                //        RAM1.ResponseScripts.Add(string.Concat(@"AbrirNotaCargoPDF('", WebURLtempPDF, "')"));
                ...
                //{
                    ...
                    Comprobante.Attributes["fecha"].Value = string.Format("{0:s}", notaCargo.Nca_Fecha);
                    ...
                    // ---------------------
                    //XmlNodeList nodosProductos = xml.SelectNodes("//Conceptos");
                    //XmlNodeList productos = xml.SelectNodes("//Concepto");
                    //XmlNode producto = productos.Item(0);
                    ...
                    //XmlNode productoDetalle = Addenda.SelectSingleNode("Detalle");
                    //string arrayProductoUnidades = string.Empty;
                    ...
                    //arrayProductoUnidades = arrayProductoUnidades.Substring(0, arrayProductoUnidades.Length - 1);
                    //productos.Item(0).RemoveAll();
                    producto.ParentNode.RemoveChild(producto);
                    //productoDetalle.ParentNode.RemoveChild(productoDetalle);
                    ...
                    else
                    {
                        ...
                        // ---------------------------------------------------------------------------------------------
                        // Ae actualiza el estatus de la nota de cargo a Impreso (I)
                        // ---------------------------------------------------------------------------------------------
                        verificador = 0;
        ...
        private void ValidarPermisos()
        {
            try
            {
                ...
                //{
                _PermisoGuardar = true;// Permiso.PGrabar;
                _PermisoModificar = true;//Permiso.PModificar;
                _PermisoEliminar = true;//Permiso.PEliminar;
                _PermisoImprimir = true;//Permiso.PImprimir;
