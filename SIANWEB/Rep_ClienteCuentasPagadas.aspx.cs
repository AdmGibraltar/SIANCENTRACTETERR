...
namespace SIANWEB
{
    public partial class Rep_ClienteCuentasPagadas : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                CN_ClientesMayorAdeudo.RegresaCuentasPagadas(Sesion.Fecha_Cierre, Sesion.Dias_Revision, Sesion.Id_Cte, Sesion.Emp_Cnx, ref Lista);
                int i = 1;
                ...
                foreach (CobSaldosNiveles saldo in Lista) {
                    carteratotal = carteratotal + saldo.Pagado;
                }
                DataTable dt = new DataTable();
                foreach (CobSaldosNiveles saldo in Lista)
                {
                    varporcentaje = (saldo.Pagado * 100) / carteratotal;
                    PieSeriesItem si = new PieSeriesItem(Math.Round((decimal)saldo.Pagado, 3), Regresacolor(saldo.Cdi), (saldo.Cdi + " - " + saldo.Cd_Nombre) + ' ' + Math.Round(varporcentaje, 1).ToString() + " %");
                    ds.SeriesItems.Add(si);
                    i++;   
                }

                RcGrafica.ChartTitle.Text = Sesion.Cte_Nombre + " Cuentas cobradas del cliente " + carteratotal.ToString("C2");
                ds.LabelsAppearance.Position = Telerik.Web.UI.HtmlChart.PieLabelsPosition.Column;
                ds.LabelsAppearance.DataFormatString = "{0:N2}";
                ds.TooltipsAppearance.Visible = false;
        ...
        protected Color Regresacolor(int cdi)
        {
        ...
        public void DetalleFacturas(string cdi) 
        {
            try 
            {
                ...
                Type instance = null;
                instance = typeof(LibreriaReportes.Rep_ClienteMayorAdeudoDoc);
