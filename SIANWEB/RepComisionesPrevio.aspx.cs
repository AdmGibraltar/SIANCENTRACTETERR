...
namespace SIANWEB
{
    public partial class RepComisionesPrevio : System.Web.UI.Page
    {
        #region Variables
            ...
            try
            {
                ...
                else
                {
                    if (!Page.IsPostBack)
                    {
                        ...
                        CN_Comun2.LlenaCombo(1,Sesion.Emp_Cnx, "SpCatCdi_Combo", ref CmbId_Cd);
        ...
        private void GenerarExcel()
        {
            ...
            string rutaynombre = "";
            String filename = "RepComisiones" + "_" + anio.ToString()+ mes.ToString()+ "_" + DateTime.Now.Year.ToString() + DateTime.Now.Month.ToString() + DateTime.Now.Day.ToString() + numero.ToString();
		...
		static void GeneraExcel(DataTable tabla, DataTable tablaDetCte, DataTable tablaDetFac, DataTable tablaDetPrd, string NombreMes)
		{
			...
			using (DbConnection connection = factory.CreateConnection())
			{
				...
				using (DbCommand command = connection.CreateCommand())
				{
                    ...
                    //command.CommandText = "CREATE TABLE [Resumen] (CDI number, NombreCD char(150), Año number, Mes number ,Rik Number, NombreRik char(150),  VtaCob number, UP number,Amortizacion number, TecSer number,  Utrem number ,Comision  number ,MVI number  )";
                    command.CommandText = "CREATE TABLE [Resumen] (CDI number, NombreCD char(150), Rik Number, NombreRik char(150),  VtaCob number, UP number,AN number, GST number,AjCobranza number ,ComPreliminar number,CND number,  GtoAdmin number,  ComisionNeta  number ,MVI number  )";
					...
					for (int j = 0; j < tabla.Rows.Count; j++)
					{
                        ...
                        comando = comando + tabla.Rows[j]["VtaCob"].ToString() + ",";
                        comando = comando + tabla.Rows[j]["UP"].ToString() + ",";

                        comando = comando + tabla.Rows[j]["Amortizacion"].ToString() + ",";
                        comando = comando + tabla.Rows[j]["GST"].ToString() + ",";
                       // comando = comando + tabla.Rows[j]["CB"].ToString() + ",";

                        comando = comando + tabla.Rows[j]["AjCobranza"].ToString() + ",";
                          comando = comando + tabla.Rows[j]["CP"].ToString() + ",";
                          comando = comando + tabla.Rows[j]["CND"].ToString() + ",";
                          comando = comando + tabla.Rows[j]["GtoAdmin"].ToString() + ","; 
					...
					for (int jr = 0; jr < tablaDetCte.Rows.Count; jr++)
					{
                        ...
                        descripcion = tablaDetCte.Rows[jr]["Nombre_Empleado"].ToString();
				...
				#region Detalle Factura
					...
					for (int jr = 0; jr < tablaDetFac.Rows.Count; jr++)
					{
                        ...
                        descripcion = tablaDetFac.Rows[jr]["Nombre_Empleado"].ToString();
                        descripcion = descripcion.Replace("\"", " ");
                        comando = comando + '\"' +  descripcion  + '\"' + ",";
                    ...
                    for (int jr = 0; jr < tablaDetPrd.Rows.Count; jr++)
                    {
                        ...
                        descripcion = tablaDetPrd.Rows[jr]["Nombre_Empleado"].ToString();
                        descripcion = descripcion.Replace("\"", " ");
                        comando = comando + '\"' + descripcion + '\"' + ",";
