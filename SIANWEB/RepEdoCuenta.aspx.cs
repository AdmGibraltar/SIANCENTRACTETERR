...
namespace SIANWEB
{
    public partial class RepEdoCuenta : System.Web.UI.Page
    {
        #region Variables
        ...
        protected void RblTipo_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    this.trCliente.Visible = true;
                    this.trCNAg.Visible = false;
                    this.TxtCte.Text = string.Empty;

                }
                else if(this.RblTipo.SelectedValue == "2")
                {
                    this.trCliente.Visible = false;
                    this.trCNAg.Visible = true;
                    this.LblCtaNacAg.Text = "Cuenta nacional";
                    CargarCuentasNacionales();
                }
                else if (this.RblTipo.SelectedValue == "3")
                {
                    this.trCliente.Visible = false;
                    this.trCNAg.Visible = true;
                    this.LblCtaNacAg.Text = "Agrupador";
                    CargarAgrupadores();
                }
            }
            catch (Exception ex)
        ...
        private void Imprimir()
        {
            try
            {
                ...
                if (this.RblTipo.SelectedValue == "1")
                {
                    ...
                    if (this.TxtCte.Text == string.Empty)
                    {
                ...
                {
                    if (this.CmbFiltro.SelectedValue == "-1")
                    {
                        Alerta("Seleccione una cuenta nacional");
                        return;
                    }
                }
                else if (this.RblTipo.SelectedValue == "3")
                ...
                ALValorParametrosInternos.Add(this.RblTipo.SelectedValue == "1" ? this.TxtCte.Text : this.CmbFiltro.SelectedValue);
                ALValorParametrosInternos.Add(this.RblTipo.SelectedValue == "1" ? this.TxtCte.Text : this.CmbFiltro.Text);
                Type instance = null;

                instance = typeof(LibreriaReportes.EstadoCuenta);
        ...
        private void CargarCuentasNacionales()
        {
            try
            {
                Sesion Sesion = new Sesion();
                ...
                CN_Comun.LlenaCombo(Sesion.Id_Emp, conexion, "spCatCuentaCorp_Combo", ref this.CmbFiltro);
            }
            catch (Exception ex)
            {
        ...
        private void CargarAgrupadores()
        {
            try
            {
                ...
                Sesion = (Sesion)Session["Sesion" + Session.SessionID];
                CapaNegocios.CN__Comun CN_Comun = new CapaNegocios.CN__Comun();
                CN_Comun.LlenaCombo(Sesion.Emp_Cnx, "SpCatAgrupadorEspecial_Combo", ref this.CmbFiltro);
            }
            catch (Exception ex)
            {
