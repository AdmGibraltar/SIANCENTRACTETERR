...
namespace SIANWEB
{
    public partial class wfrmInformes : System.Web.UI.Page
    {
        ...
        public void ExportarControlPromocion()
        {
            ...
            else
            {
                ...
                else
                {
                    if (File.Exists(ruta))
                        File.Delete(ruta);
                    if (File.Exists(Server.MapPath("Reportes") + "\\Reporte" + this.session.Id_U.ToString() + tipo + ".xls"))
                        File.Delete(Server.MapPath("Reportes") + "\\Reporte" + this.session.Id_U.ToString() + tipo + ".xls");
                    ...
                    if ((dsReporte != null))
                    {
                        if (!(dsReporte.Tables[0].Rows.Count == 0))
                        {
                            try
                            {
                                ...
                                {
                                    string ruta2 = null;
                                    ruta2 = Server.MapPath("Reportes") + "\\Reporte" + this.session.Id_U.ToString() + tipo + ".xls";
                                    File.Move(ruta, Server.MapPath("Reportes") + "\\Reporte" + this.session.Id_U.ToString() + tipo + ".xls");
                                    Response.Redirect("Reportes\\Reporte" + this.session.Id_U.ToString() + tipo + ".xls");
                                }
                            }
                            catch (Exception Ex)
                            {
                ...
                if (!(dsCE.Tables[0].Rows.Count == 0))
                {
                    ...
                    if ((Session["dtRepresentantes"] != null))
                    {
                        ...
                        if (File.Exists(ruta))
                        {
                            ...
                            ruta2 = Server.MapPath("Reportes") + "\\Reporte" + this.session.Id_U.ToString() + tipo + ".xls";
                            File.Move(ruta, Server.MapPath("Reportes") + "\\Reporte" + this.session.Id_U.ToString() + tipo + ".xls");
                            Response.Redirect("Reportes\\Reporte" + this.session.Id_U.ToString() + tipo + ".xls", false);
                        }
                    }
        ...
        private void ExportarCampania()
        {
            ...
            if ((dsCE != null))
            {
                if (!(dsCE.Tables[0].Rows.Count == 0))
                {
                    ...
                    sw.WriteLine("<td bgcolor=\"#DDDDDD\"><b>Cantidad</td></tr>");
        ...
        public void CrearFilasCampania(int id_cam,string Campana, string CDS, string UsuarioID, string Representante, string Uen, string Segmento, string Area, string Solucion, string Aplicacion, string Producto, int Cantidad, float Efectividad)
        {
            DataTable dtRepresentantes = new DataTable();
            dtRepresentantes = (DataTable)Session["dtRepresentantes"];
            DataRow drFila = null;
        ...
        protected void RadAjaxManager1_AjaxRequest(object sender, AjaxRequestEventArgs e)
        {
            try
            {
                ...
                if (session.Id_TU == 2)
                {
                    this.radDII.Style.Add("display", "none");//
                    this.radDIINumero.Style.Add("display", "none");//
                   // this.radControlEntrada.Style.Add("display", "none");
                    this.radCampania.Style.Add("display", "none");
                }
                this.CargarZonas();
                this.CargarRepresentantes();
