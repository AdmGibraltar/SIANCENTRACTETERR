...
namespace SIANWEB
{
    public partial class Rep_InvTransito : System.Web.UI.Page
    {
        #region Variables
        ...
        private void Imprimir(bool a_pantalla)
        {
            try
            {
                string paramProd = "";
                string paramEntrega = "";
                string paramTransporte = "";
                string strTipo = "";
                int tipo = 0;
                int error = 0;
                if (!string.IsNullOrEmpty(this.txtProductos.Text))
                {
                    boton(this.txtProductos.Text, ref error);
                    paramProd = this.txtProductos.Text;
                }
                if (!string.IsNullOrEmpty(this.txtTEntrega.Text))
                {
                    boton(this.txtTEntrega.Text, ref error);
                    paramEntrega = this.txtTEntrega.Text;
                }
                if (!string.IsNullOrEmpty(this.txtTTransporte.Text))
                {
                    boton(this.txtTTransporte.Text, ref error);
                    paramTransporte = this.txtTTransporte.Text;
                }
                if (RBTodos.Checked)
                {
                    tipo = 0;
                    strTipo = RBTodos.Text;
                }
                if (RBTransito.Checked)
                {
                    tipo = 1;
                    strTipo = RBTransito.Text;
                }
                if (RBTiempoEnt.Checked)
                {
                    tipo = 2;
                    strTipo = RBTiempoEnt.Text;
                }
                if (RBTiempoTran.Checked)
                {
                    tipo = 3;
                    strTipo = RBTiempoTran.Text;
                }
                if (error == 0)
                {
                    ...
                    ALValorParametrosInternos.Add(this.txtProductos.Text == "" ? "Todos" : this.txtProductos.Text); // OPCION DE SELECCION POR PRODUCTO
                    ALValorParametrosInternos.Add(this.txtTEntrega.Text == "" ? "Todos" : this.txtTEntrega.Text); // OPCION DE SELECCION POR TENTREGA
                    ALValorParametrosInternos.Add(this.txtTTransporte.Text == "" ? "Todos" : this.txtTTransporte.Text); // OPCION DE SELECCION POR TTRANSPORTE
                    ALValorParametrosInternos.Add(strTipo); // OPCION DE SELECCION POR TIPO
                    ...
                    ALValorParametrosInternos.Add(this.txtProductos.Text == "" ? "" : this.txtProductos.Text); // OPCION DE SELECCION POR RELACIONES
                    ALValorParametrosInternos.Add(this.txtTEntrega.Text == "" ? "" : this.txtTEntrega.Text); // OPCION DE SELECCION POR RELACIONES
                    ALValorParametrosInternos.Add(this.txtTTransporte.Text == "" ? "" : this.txtTTransporte.Text); // OPCION DE SELECCION POR RELACIONES
                    ALValorParametrosInternos.Add(tipo); // OPCION DE SELECCION POR TIPO
                    ALValorParametrosInternos.Add(sesion.Emp_Cnx); // CADENA DE CONEXION A LA BASE DE DATOS

                    Type instance = null;
                    if (a_pantalla)
                    {
                        instance = typeof(LibreriaReportes.RepTransito);
                    }
                    else
                    {
                        instance = typeof(LibreriaReportes.ExpRepTransito);
                    }
        ...
        protected void RBTransito_CheckedChanged(object sender, EventArgs e)
        {
