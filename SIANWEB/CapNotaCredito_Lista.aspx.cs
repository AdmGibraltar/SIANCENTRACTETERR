...
namespace SIANWEB
{
    public partial class CapNotaCredito_Lista : System.Web.UI.Page
    {
                        ...
                        case "Imprimir":
                            ...
                            if ((estatus.Contains("Impreso") || estatus.Contains("Capturado")))
                            {
                                if (_PermisoImprimir)
                                {
                                    ...
                                    if (Verificador == -1)
                                    {
                                        this.ImprimirNotaCredito(Id_Emp, Id_Cd, Id_Ncr, Id_NcrSerie );
                                    }
        ...
        private void ImprimirNotaCredito(int Id_Emp, int Id_Cd, int Id_Ncr, string Id_NcrSerie)
        {
            try
            {
                Sesion sesion = (Sesion)Session["Sesion" + Session.SessionID];
                ...
                new CN_CapNotaCredito().ConsultarAdenda(Id_Emp, Id_Cd, Id_Ncr, "5", "6", ref listCabT, ref listDetT, sesion.Emp_Cnx);
                ...
                Comprobante.Attributes["fecha"].Value = string.Format("{0:s}", notaCredito.Ncr_Fecha);
                ...
                if (notaCredito.Ncr_Movimiento != null)
                {
                    if (Convert.ToInt32(notaCredito.Ncr_Movimiento) == 1)
                    {
                        ...
                        factura.Id_FacSerie = Cfe_NombreAcuse + notaCredito.Ncr_Referencia.ToString();
