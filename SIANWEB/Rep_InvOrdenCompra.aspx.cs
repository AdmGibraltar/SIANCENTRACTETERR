...
namespace SIANWEB
{
    public partial class Rep_InvOrdenCompra : System.Web.UI.Page
    {
        #region Variables
        ...
        private void Imprimir(bool a_pantalla)
        {
            try
            {
                ...
                int error = 0;
                //string paramPrd = 
                this.boton(this.txtProducto.Text, ref error);
                string Estatus = "";
                ...
                ALValorParametrosInternos.Add(Estatus); //Estatus de la orden de compra
                ...
                /*   PARAMETROS QUE PUEDEN SER NULOS   */
                ALValorParametrosInternos.Add(string.IsNullOrEmpty(txtProducto.Text) ? null : txtProducto.Text); //Cadena Id's de productos
                ALValorParametrosInternos.Add(this.txtOrden.Text); //Id de orden de compra
                ALValorParametrosInternos.Add(this.sesion.Emp_Nombre); // NOMBRE DE LA EMPRESA

                Type instance = null;
                if (a_pantalla)
                {
                    instance = typeof(LibreriaReportes.Rep_InvOrdenCompra);
                }
                else
                {
                    instance = typeof(LibreriaReportes.ExpRep_InvOrdenCompra);
                }
                if (error == 0)
                {
                    if (a_pantalla)
