...
namespace SIANWEB
{
    public partial class CapGestionPreciosD : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                else
                {
                    if (!Page.IsPostBack)
                    {
                        ...
                        {
                            this.rgConvenioDet.DataSource = ConsultaConvenio();
                            rgConvenioDet.DataBind();
                            rgConvenioDet.Rebind();
                        }
        ...
        protected void rgConvenioDet_PageIndexChanged(object source, Telerik.Web.UI.GridPageChangedEventArgs e)
        {
            try
            {
                rgConvenioDet.Rebind();
        ...
        protected void rgConvenioDet_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            try
            {
        ...
        protected void rgConvenioDet_ItemCommand(object source, GridCommandEventArgs e)
        {
            try
            {
        ...
        private void Borrar(GridCommandEventArgs e)
        {
            try
            {
                GridItem gi = e.Item;
                string Id_Prd;
                Id_Prd = this.rgConvenioDet.MasterTableView.Items[e.Item.ItemIndex]["Id_Prd"].Text;

                ListDet.Remove(ListDet.Where(x => x.Id_Prd  ==  int.Parse(Id_Prd)).ToList()[0]);

                rgConvenioDet.DataSource = ListDet;
                rgConvenioDet.DataBind();
