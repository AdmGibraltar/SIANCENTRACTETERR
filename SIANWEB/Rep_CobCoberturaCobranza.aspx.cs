...
namespace SIANWEB
{
    public partial class Rep_CobCobertura : System.Web.UI.Page
    {
        #region Variables
        ...
        protected void rtb1_ButtonClick(object sender, Telerik.Web.UI.RadToolBarEventArgs e)
        {
            try
            {
                ...
                if (this.dpFechainicial.SelectedDate == null)
                {
                    this.Alerta("Seleccione una fecha inicial");
                    this.dpFechainicial.Focus();
                    return;
                }

                if (this.dpFechafinal.SelectedDate == null)
                {
                    this.Alerta("Seleccione una fecha final");
                    this.dpFechafinal.Focus();
                    return;
                }

                if (this.dpFechafinal.SelectedDate < this.dpFechainicial.SelectedDate)
                {
                    this.Alerta("La fecha final no puede ser menor que la fecha inicial");
                    this.dpFechafinal.Focus();
                    return;
                }
        ...
        private void ValidarPermisos()
        {
            try
            {
                ...
                if (sesion.CalendarioIni >= dpFechainicial.MinDate && sesion.CalendarioIni <= dpFechainicial.MaxDate)
                {
                    dpFechainicial.DbSelectedDate = sesion.CalendarioIni;
                }
                if (sesion.CalendarioFin >= dpFechafinal.MinDate && sesion.CalendarioFin <= dpFechafinal.MaxDate)
                {
                    dpFechafinal.DbSelectedDate = sesion.CalendarioFin;
                }
        ...
        private void Imprimir(bool a_pantalla)
        {
            try
            {
                ...
                ALValorParametrosInternos.Add(this.dpFechainicial.SelectedDate == null
                   ? "Todas" : Convert.ToDateTime(dpFechainicial.SelectedDate).ToString("dd/MM/yyyy"));
                ALValorParametrosInternos.Add(this.dpFechafinal.SelectedDate == null
                   ? "Todas" : Convert.ToDateTime(dpFechafinal.SelectedDate).ToString("dd/MM/yyyy"));
                ...
                if (a_pantalla)
                {
                    instance = typeof(LibreriaReportes.Rep_CobCoberturaCobranza);
                }
                else
                {
                    instance = typeof(LibreriaReportes.ExpRep_CobCoberturaCobranza);
                }
                if (_PermisoImprimir)
                {
                    if (a_pantalla)
