...
namespace CapaDatos
{
    public class CD_CatAcreedor
    {
        ...
        public void ImportarAcreedor(string Conexion, int id_emp, ref int verificador)
        {
            CapaDatos.CD_Datos CapaDatos = new CapaDatos.CD_Datos(Conexion);
            try
            {

                string[] Parametros = { "@Id_Emp", "@ImportarTodo" };
	                                 
                object[] Valores = { 
                                      id_emp,1
                                   };

                SqlCommand sqlcmd = CapaDatos.GenerarSqlCommand("SpImportaCatAcreedor", ref verificador, Parametros, Valores);
